	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 texColor(v2f i, float rim)
			{
				...
				mainTex.r *= triWave(_Time.x * 10, abs(i.objectPos.y) * 2, -0.3) * 6;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				clip(tex2D(_SliceGuide, i.uv * 3) - _SliceAmount);
				...
				float northPole = (i.objectPos.y - 0.45) * 0;
				float glow = max(max(intersect, rim), northPole);

				fixed4 glowColor = fixed4(lerp(_Color.rgb, fixed3(1, 1, 1), pow(glow, 10)), 1);
