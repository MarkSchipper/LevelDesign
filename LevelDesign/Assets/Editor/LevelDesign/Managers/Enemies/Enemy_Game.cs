    ...
    public static void ShowEditEnemy()
    {
        if (!_enemySaved)
        {
            ...
            if (GUI.changed)
            {
                ...
                _enemyDamage = _enemyCombatData.ReturnDamage();
                _enemyCooldown = _enemyCombatData.ReturnCooldown();
                ...
                _enemyAttackRange = GameObject.Find(_enemiesSorted[_editGameSelectIndex]).GetComponentInChildren<EnemyCombat.EnemyMotor>().ReturnAttackRange();
            ...
            _enemyType = _enemyCombatData.ReturnType();
