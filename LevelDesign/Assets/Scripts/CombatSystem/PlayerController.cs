...
namespace CombatSystem
{
    ...
    public class PlayerController : MonoBehaviour
    {
        ...
        [SerializeField]
        private GameObject _playerWebbed;
        ...
        private CombatSystem.PlayerBattle PLAYER_BATTLE;
        ...
        void OnEnable()
        {
            ...
            PLAYER_BATTLE = GetComponent<CombatSystem.PlayerBattle>();
        ...
        void Start()
        {
            ...
            _rotateAngle = transform.localEulerAngles.y;
            _playerWebbed.SetActive(false);
                            ...
                            if (!InteractionManager.instance.ReturnHoveringOverUI())
                            {
                                ...
                                else if (_hit.collider.tag == "EnemyMelee")
                                {
                                    ...
                                    if(_selectedActor != _hit.collider.gameObject && _selectedActor != null)
                                    {
                                        ...
                                        Debug.Log("set the enemy selected to false");
                                    }
                ...
                if (coll.name == "Enemy_MELEE_TRIGGER" && coll.GetComponentInParent<EnemyCombat.EnemyMotor>().ReturnAliveState())
                {
                    if (!_hasTakenDamage)
                    {
                        if (_playerHealth - (int)coll.GetComponentInParent<EnemyCombat.EnemyBattle>().ReturnDamage() > 0)
                        {
                            if (_hasBarrier)
                            {
                                _playerHealth -= (int)coll.GetComponentInParent<EnemyCombat.EnemyBattle>().ReturnDamage();
                            }
                            if (!_hasBarrier)
                            {
                                // Reduce the players health with the damage from the spell
        ...
        public void CastDebuff(float _time, string _prefab, DebuffAbility _debuff)
        {
            ...
            {
                Dialogue.DialogueManager.instance.ShowMessage("No Enemy Selected", true);
            }
        }
        ...
        {
            PLAYER_MOVEMENT.Knockback(dir, force);
        }

        public void PlayerWebbed(float duration)
        {
            INPUT_BLOCK = true;
            _playerWebbed.SetActive(true);

            foreach (SkinnedMeshRenderer skin in GetComponentsInChildren<SkinnedMeshRenderer>())
            {
                skin.enabled = false;
            }
            _playerWebbed.GetComponentInChildren<SkinnedMeshRenderer>().enabled = true;
            StartCoroutine(RestorePlayerControl(2f));
            StartCoroutine(HidePlayerWebbed(3f));
        }

        public GameObject ReturnPlayerWebbed()
        {
            return _playerWebbed;
        }

        public void PlayerStunned(float duration)
        {
            INPUT_BLOCK = true;
        }
        ...
        IEnumerator RestorePlayerControl(float timer)
        {
            ...
            //_playerWebbed.GetComponentInChildren<SkinnedMeshRenderer>().enabled = false;

        }

        IEnumerator HidePlayerWebbed(float _time)
        {
            yield return new WaitForSeconds(_time);
            _playerWebbed.SetActive(false);
        }
        ...
        // passing to the battle system

        public void CreateBarrier(float _value)
        {
            PLAYER_BATTLE.CreateBarrier(_value);
        }
