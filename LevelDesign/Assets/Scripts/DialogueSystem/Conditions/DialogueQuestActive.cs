...
namespace Dialogue
{
    public class DialogueQuestActive : MonoBehaviour
    {
        public static void QuestActive(int _npcID, int _nodeID, GameObject _selectedNPC, int _answer)
        {
            Quest.QuestGameManager.GetAllQuests();

            // check if quest is active
            if (Quest.QuestGameManager.ReturnQuestActive(Quest.QuestGameManager.ReturnQuestID()[int.Parse(Dialogue.Game.DialogueGameDatabase.GetConditionValue(_npcID, Dialogue.Game.DialogueGameDatabase.GetNodeIDsByAnswer(_npcID, _nodeID)[0]))]))
            {
