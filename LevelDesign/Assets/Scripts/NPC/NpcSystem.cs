...
namespace NPC
{
    ...
    public class NpcSystem : MonoBehaviour
    {
        ...
        [SerializeField] private float _waypointIdleTime;
        ...
        public void SetData(int id, string name, ActorBehaviour behaviour, float patrol, string prefab, float _waitTime)
        {
            _npcID = id;
            _npcName = name;
        ...
        void Patrol()
        {
            if (_wayPoints.Count > 0)
            {
                if (_currentWayPoint < _wayPoints.Count)
                {
                    ...
                    {
                        if (_waypointIdleTime > 0)
                        {
                            StartCoroutine(PatrolIdleTimer(_waypointIdleTime));
                            _currentWayPoint++;
                        }
                        else
                        {
        ...
        void CreateDialogue()
        {
            if (STATE_CONVERSATION)
            {
                if (_setInteraction)
                {
                    ...
                    Dialogue.DialogueManager.instance.InitiateDialogue(_npcID, false, this.gameObject);
        ...
        IEnumerator StopConversation()
        {
            ...
            Dialogue.DialogueManager.instance.CancelDialogue();
            SwitchOnNpcCamera(false);
            CombatSystem.PlayerController.instance.SetInputBlock(false);
        }

        IEnumerator PatrolIdleTimer(float _timer)
        {
            STATE_IDLE = true;
            STATE_PATROL = false;
